...
#
#     # blow is only for penn group to load lightweight model
#     args = parser.parse_args()
#     args.use_penn = True
#     args.multiple_outputs = True
#     args.refine = True
#     args.refine_test = True
#     pretrained_path = '/media/hao/mydrive1/download/best.pth.tar'
#
#     sam = _build_sam3D_ori(
#         encoder_embed_dim=768,
#         encoder_depth=12,
#         encoder_num_heads=12,
#         encoder_global_attn_indexes=[2, 5, 8, 11],
...
def _build_sam3D_ori(
    ...
    if args.use_penn:
        # Only load mask decoder when use_penn is enabled
        sam = sam3D.Sam3D(
            image_encoder=None,  # Skip loading image encoder
            prompt_encoder=None,  # Skip loading prompt encoder
            mask_decoder=mask_decoder_use_penn.MaskDecoder3D(
                args,
                transformer_dim=prompt_embed_dim,
                num_multiple_outputs=args.num_multiple_outputs,
    ...
    if checkpoint is not None:
        ...
        if args.use_penn:
            # Only load weights for mask_decoder
